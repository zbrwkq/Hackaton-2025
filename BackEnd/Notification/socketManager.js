...
const initSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    console.log(
      `🔍 Détails de la connexion: transport=${socket.conn.transport.name}, adresse=${socket.handshake.address}`
    );
    ...
    socket.on("disconnect", (reason) => {
      ...
      if (disconnectedUser) {
        ...
        console.log(
          `❌ Socket déconnecté sans utilisateur associé: ${socket.id}`
        );
      }
    ...
    socket.on("register", (userId) => {
      if (!userId) {
        console.error(
          "❌ Tentative d'enregistrement avec un ID utilisateur vide ou null"
        );
        return;
      }
...
const sendNotification = (userId, notificationData) => {
  ...
  setTimeout(() => {
    ...
    if (!notificationData._id) {
      console.error(
        "🚨 ALERTE : La notification envoyée à WebSocket n'a pas d'_id !"
      );
    }
    ...
    if (socketId) {
      ...
      try {
        ...
        console.error(
          `❌ Erreur lors de l'envoi de la notification à ${userKey}:`,
          error
        );
      }
    } else {
